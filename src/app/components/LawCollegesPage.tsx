...
import { MapPin, Star, Users, Award, ArrowRight, Search, TrendingUp, Sparkles, Filter, X, Scale } from 'lucide-react';
...
export function LawCollegesPage() {
  ...
  const lawColleges = colleges.filter(college => {
    const hasLaw = college.courses?.some(course => 
      course.includes('LL.B') || course.includes('Law') || course.includes('LLB')
    );
    return hasLaw;
  });

  let filteredColleges = lawColleges.filter(college => {
  ...
  return (
    <>
      <Helmet>
        <title>Top Law Colleges in Noida 2026 - LL.B, BA LLB | Dreamz College</title>
        <meta name="description" content="Explore top law colleges in Noida & Greater Noida 2026. LL.B, BA LLB colleges with fees, placement, CLAT cutoff. Get free counseling for law admission!" />
        <meta name="keywords" content="law colleges in noida, llb colleges in noida, ba llb colleges in up, best law colleges in delhi ncr" />
      </Helmet>
        ...
        <div className="pt-20 pb-16">
          <div className="text-center max-w-4xl mx-auto px-4 mb-12">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-2 bg-purple-100 text-purple-700 rounded-full text-sm font-semibold mb-4">
              <Scale className="w-4 h-4" /> Top Law Colleges 2026
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-4">
              Best <span className="bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">Law Colleges</span> in Noida
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-lg text-gray-600 max-w-2xl mx-auto">
              Find the best LL.B, BA LLB colleges in Noida & Greater Noida. Compare fees, CLAT cutoff, and choose your dream law college.
            </motion.p>
          </div>

          <div className="max-w-6xl mx-auto px-4 mb-12">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl p-4 shadow-md text-center"><div className="text-2xl font-bold text-purple-600">{lawColleges.length}+</div><div className="text-xs text-gray-500">Law Colleges</div></div>
              <div className="bg-white rounded-2xl p-4 shadow-md text-center"><div className="text-2xl font-bold text-blue-600">5+</div><div className="text-xs text-gray-500">Programs</div></div>
          ...
          <div className="max-w-6xl mx-auto px-4 mb-8">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Search law colleges..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white" /></div>
          ...
          <div className="max-w-6xl mx-auto px-4">
            ...
            {filteredColleges.length === 0 && (<div className="text-center py-12"><p className="text-gray-500">No law colleges found.</p><button onClick={clearFilters} className="mt-4 px-6 py-2 bg-purple-600 text-white rounded-lg">Clear Filters</button></div>)}
          </div>

          <div className="max-w-6xl mx-auto px-4 mt-12"><div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-8 text-white text-center"><h2 className="text-2xl md:text-3xl font-bold mb-2">Need Help Choosing the Right Law College?</h2><p className="mb-4 opacity-90">Get free expert counseling from our career advisors</p><Link to="/counseling" className="inline-flex items-center gap-2 bg-white text-purple-600 px-8 py-3 rounded-full font-semibold hover:shadow-lg transition-all">Get Free Counseling <ArrowRight className="w-4 h-4" /></Link></div></div>
        </div>
      </div>
    </>
  );
