...
import { 
  ...
  Stethoscope, Activity, Bone, Scale, Gavel
} from 'lucide-react';
...
export function InnovativeGroupPage() {
  ...
  const college = {
    id: 'innovative-group',
    name: 'Innovative Group of Colleges',
    fullName: 'Innovative Group of Colleges, Greater Noida',
    location: '6, Knowledge Park II, Greater Noida, Uttar Pradesh - 201310',
    rating: 4.1,
    students: '3,500+',
    ...
    accreditation: ['AICTE', 'NAAC', 'NBA', 'PCI', 'BCI', 'NCTE'],
    ...
    facilities: [
      ...
      'Moot Court', 'Pharmacy Labs', 'Biotechnology Labs', 'Herbal Garden',
      'Cafeteria', 'Transport Facility', 'Medical Facility', 'Gymnasium',
      'Bank/ATM', 'Temple', 'Indoor Games Zone', 'Cricket Academy'
    ],
    contact: {
      phone: '8448448851',
      email: 'info@innovativegroupofcolleges.com',
      website: 'www.innovativegroupofcolleges.com',
      tollFree: '1800-123-4567',
      helpline: '8448448851'
    },
    fees: '₹31,000 - ₹1,60,000/year',
    reviews: 1250,
    description: 'Innovative Group of Colleges, established in 2005, is a premier multi-disciplinary institute offering quality education in Pharmacy, Law, Management, IT, Biotechnology, and Education. With AICTE, PCI, BCI, and NCTE approvals, the institute is known for its state-of-the-art infrastructure, experienced faculty, and excellent placement record. The campus features a moot court, advanced labs, herbal garden, and a cricket academy in association with former Indian cricketers.',
    specializations: [
      'Pharmaceutics', 'Pharmacology', 'Pharmaceutical Chemistry', 'Pharmacognosy',
      'Criminal Law', 'Corporate Law', 'Constitutional Law', 'Family Law',
      'Marketing', 'Finance', 'Human Resource', 'International Business',
      'Web Development', 'Data Science', 'Cyber Security'
    ],
    brochure: '/brochures/innovative-group.pdf',
    achievements: [
      'PCI Approved',
      'BCI Approved',
      'NCTE Approved',
      ...
      'Cricket Academy with Former Indian Cricketers'
    ],
    alumniCount: '8,000+',
    facultyCount: '200+',
    globalPrograms: [
    ...
    faqs: [
      { question: 'What is the eligibility for B.Pharm at Innovative Group?', answer: '10+2 with PCB/PCM with minimum 50% marks. UPSEE qualified candidates are preferred.' },
      { question: 'What is the fee structure for B.Pharm?', answer: 'B.Pharm fee is ₹1,29,000 per year. Total fee for 4 years is approximately ₹5.16 Lakhs.' },
      { question: 'Does Innovative Group have hostel facilities?', answer: 'Yes, separate hostels for boys and girls with AC and Non-AC rooms. Hostel fees range from ₹1,10,000 to ₹1,45,000 per year.' },
      { question: 'What is the placement record of Innovative Group?', answer: '80% placement record with highest package ₹25 LPA and average package ₹4.5 LPA. Top recruiters include TCS, Wipro, Infosys, Cipla, Sun Pharma.' },
      { question: 'Is Innovative Group approved by AICTE?', answer: 'Yes, Innovative Group is approved by AICTE, PCI, BCI, and NCTE.' },
      { question: 'What are the law programs offered?', answer: 'Innovative Group offers LL.B (3 years), B.A. LL.B (5 years), and B.Com LL.B (5 years) with BCI approval.' },
      { question: 'Does Innovative Group offer scholarship?', answer: 'Yes, merit-based scholarships available for students with good scores in qualifying examination.' },
      { question: 'What is the Cricket Academy?', answer: 'Innovative Group has a cricket academy in association with former Indian cricketers Saba Karim and Rajesh Chauhan.' },
      { question: 'What is the admission process for LL.B?', answer: 'Admission to LL.B is based on merit in graduation and CLAT score. Minimum 45% in graduation required.' },
      { question: 'What is the alumni network strength?', answer: 'Innovative Group has an alumni network of 8,000+ professionals working in top companies across India.' }
    ]
  };
  ...
  const getCourseDetails = (courseName: string) => {
    ...
    if (courseName.includes('LL.B') && !courseName.includes('B.A.') && !courseName.includes('B.Com')) return college.courseDetails['LL.B'];
    if (courseName.includes('B.A. LL.B')) return college.courseDetails['B.A. LL.B'];
    if (courseName.includes('B.Com LL.B')) return college.courseDetails['B.Com LL.B'];
  ...
  const submitToGoogleSheets = async (data: any) => {
    try {
      ...
      formData.append('timestamp', new Date().toISOString()); formData.append('source', 'Innovative Group - Apply Now');
  ...
  const facilityIcons: { [key: string]: any } = {
    ...
    'Seminar Halls': UsersRound, 'Moot Court': Gavel, 'Pharmacy Labs': FlaskConical,
    'Biotechnology Labs': Microscope, 'Herbal Garden': Flower, 'Cafeteria': Coffee,
    'Transport Facility': Bus, 'Medical Facility': Heart, 'Gymnasium': Dumbbell,
    'Bank/ATM': Shield, 'Temple': Building2, 'Indoor Games Zone': Gamepad, 'Cricket Academy': Activity
  };

  const pharmacyCourses = college.courses.filter(c => c.includes('Pharm'));
  const lawCourses = college.courses.filter(c => c.includes('LL.B'));
  const managementCourses = college.courses.filter(c => c.includes('BBA'));
  ...
  return (
    <>
      ...
      <meta name="description" content={`${college.name} - Explore fees (${college.fees}), placement packages (${college.highestPackage}), courses offered (B.Pharm, LL.B, BBA, BCA, B.Sc Biotech, B.Ed). PCI | BCI | NCTE Approved. Get free counseling for 2026 admissions!`} /></Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2 space-y-6">
              ...
              {activeTab === 'courses' && (
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 rounded-xl p-4"><p className="text-sm text-gray-600"><span className="font-semibold">Note:</span> Hostel fees: ₹1,10,000 - ₹1,45,000/year. Scholarships available for meritorious students. Cricket Academy in association with former Indian cricketers Saba Karim and Rajesh Chauhan.</p></div>
                </div>
              )}
              ...
              {activeTab === 'contact' && (
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  ...
                  <div className="mt-6"><h3 className="font-semibold mb-2">Location</h3><div className="bg-gray-100 rounded-lg h-48"><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3506.774!2d77.496!3d28.501!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMjnCsDMwJzAzLjYiTiA3N8KwMjknMzQuNyJF!5e0!3m2!1sen!2sin!4v1" width="100%" height="100%" style={{border:0}} allowFullScreen loading="lazy" title="Innovative Group Location"></iframe></div><p className="text-sm text-gray-500 mt-2">{college.location}</p></div>
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-1"><div className="sticky top-16 space-y-6">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border"><h3 className="font-semibold mb-3">Quick Facts</h3><div className="space-y-2 text-sm"><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Established</span><span className="font-medium">{college.established}</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Approvals</span><span className="font-medium">PCI, BCI, NCTE, AICTE</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Student Strength</span><span className="font-medium">{college.students}</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Faculty Members</span><span className="font-medium">{college.facultyCount}</span></div><div className="flex justify-between py-2"><span className="text-gray-500">Alumni Network</span><span className="font-medium">{college.alumniCount}</span></div></div></div>
            </div></div>
          </div>
        </div>
