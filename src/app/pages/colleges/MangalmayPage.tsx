...
export function MangalmayPage() {
  ...
  const college = {
    ...
    established: '2002',
    affiliation: 'AKTU, Lucknow | CCS University, Meerut',
    accreditation: ['NAAC A Grade', 'AICTE', 'NBA', 'PCI', 'NCTE'],
    ...
    courseDetails: {
      'B.Tech CSE': {
        ...
        totalFees: '₹6,60,000 (Total for 4 years)',
        seats: 180,
        ...
        highlights: ['NBA Accredited', 'Advanced Computing Labs', 'Industry Certifications']
      },
      'B.Tech CSE DS': {
      ...
      'MBA++': {
        ...
        fees: '₹2,62,500/year',
        perSemesterFees: '₹1,31,250/semester',
        totalFees: '₹5,25,000 (Total for 2 years)',
        seats: 60,
        specializations: ['Marketing', 'Finance', 'Human Resource', 'Business Analytics', 'Operations', 'International Business'],
      ...
      'MBA': {
        ...
        highlights: ['AICTE Approved', 'Industry Oriented Curriculum', 'Case Based Teaching']
      },
      'BBA': {
        ...
        fees: '₹84,000/year',
        perSemesterFees: '₹42,000/semester',
        totalFees: '₹2,52,000 (Total for 3 years)',
        seats: 180,
      ...
      'BCA': {
        ...
        totalFees: '₹3,87,000 (Total for 3 years)',
        seats: 120,
      ...
      'D.Pharm': {
        ...
        totalFees: '₹1,10,000 (Total for 2 years)',
        seats: 60,
      ...
      'B.A.B.Ed': {
        ...
        fees: '₹55,000/year',
        perSemesterFees: '₹27,500/semester',
        totalFees: '₹2,20,000 (Total for 4 years)',
        seats: 100,
    ...
    placementRate: '92%',
    topRecruiters: [
      'Publicis Sapient', 'Amazon', 'TCS', 'Deloitte', 'KPMG', 'LG Electronics',
      'Cipla', 'HCL', 'IBM', 'Microsoft', 'HDFC Bank', 'ICICI Bank', 'Wipro',
      'Infosys', 'Accenture', 'Tech Mahindra', 'Cognizant', 'Capgemini'
    ],
    facilities: [
      'Smart Classrooms', 'Incubation Center', 'Research Labs', 'Computer Labs',
      'Digital Library', 'Hostel', 'Sports Complex', 'NCC Wing', 'Innovation Hub',
      'Cafeteria', 'Transport Facility', 'Medical Facility', 'Gymnasium', 'Bank/ATM'
    ],
    contact: {
      phone: '1800-103-3797',
      email: 'admission@mangalmay.org',
      website: 'www.mangalmay.org',
      tollFree: '1800 103 3797'
    },
    fees: '₹55,000 - ₹2,62,500/year',
    reviews: 980,
    description: 'Mangalmay Group offers NAAC A Grade accredited programs in Engineering, Management, Pharmacy, and Education. The institute has collaborations with IIT Kanpur, IIT Kharagpur, IBM, and TCS for certification programs. With state-of-the-art infrastructure and excellent placement record, Mangalmay is a top choice for students in Greater Noida.',
    specializations: [
      'Computer Science', 'Artificial Intelligence', 'Data Science',
      'Marketing', 'Finance', 'HR', 'International Business',
      'Pharmaceutics', 'Pharmacology', 'Teaching Methodology'
    ],
    brochure: '/brochures/mangalmay.pdf',
    achievements: [
      'NAAC A Grade',
      'IIM Bangalore Collaboration',
      'Best Emerging Institute 2024',
      'Top Placement Record',
      'AICTE Approved',
      'PCI Approved for Pharmacy',
      'NCTE Approved for Education'
    ],
    alumniCount: '15,000+',
    facultyCount: '400+',
    globalPrograms: [
    ...
    faqs: [
      { question: 'What is the eligibility for MBA++ program?', answer: 'Graduation in any discipline with minimum 50% marks. MAT/CMAT/CAT/XAT scores accepted.' },
      { question: 'What is the total fee for MBA++?', answer: 'MBA++ total fee is ₹5,25,000 for two years (₹2,62,500 per year).' },
      { question: 'Does Mangalmay provide hostel facilities?', answer: 'Yes, hostel available for boys and girls. 2-seater: ₹1,15,000/year, 3-seater: ₹1,00,000/year.' },
      { question: 'What is the placement record?', answer: '92% placement record with highest package ₹25 LPA and average ₹6.8 LPA.' },
      { question: 'Is Mangalmay NAAC accredited?', answer: 'Yes, Mangalmay is NAAC A Grade accredited.' },
      { question: 'What are the specializations in B.Tech?', answer: 'B.Tech offered in CSE, CSE (Data Science), and CSE (Artificial Intelligence).' },
      { question: 'Does Mangalmay have IIT collaborations?', answer: 'Yes, collaborations with IIT Kanpur, IIT Kharagpur, IBM, and TCS for certifications.' },
      { question: 'What is the admission process for BBA?', answer: 'Merit-based on 10+2 marks. Minimum 50% required.' },
      { question: 'Are there scholarship options?', answer: 'Yes, scholarships up to ₹60,000 based on entrance exam scores and academic performance.' },
      { question: 'What is the alumni strength?', answer: 'Mangalmay has 15,000+ alumni working in top MNCs globally.' }
    ]
  };

  const getCourseDetails = (courseName: string) => {
    if (courseName.includes('B.Tech CSE') && !courseName.includes('Data') && !courseName.includes('Artificial')) return college.courseDetails['B.Tech CSE'];
    if (courseName.includes('Data Science')) return college.courseDetails['B.Tech CSE DS'];
    if (courseName.includes('Artificial Intelligence')) return college.courseDetails['B.Tech CSE AI'];
    if (courseName.includes('MBA++')) return college.courseDetails['MBA++'];
    if (courseName.includes('MBA') && !courseName.includes('++')) return college.courseDetails['MBA'];
    ...
    if (courseName.includes('B.A.B.Ed')) return college.courseDetails['B.A.B.Ed'];
    return null;
  };
  ...
  const submitToGoogleSheets = async (data: any) => {
    try {
      ...
      formData.append('timestamp', new Date().toISOString()); formData.append('source', 'Mangalmay - Apply Now');
  ...
  const facilityIcons: { [key: string]: any } = {
    'Smart Classrooms': GraduationCap, 'Incubation Center': Rocket, 'Research Labs': Microscope,
    'Computer Labs': Laptop, 'Digital Library': Library, 'Hostel': Home, 'Sports Complex': Dumbbell,
    'NCC Wing': Shield, 'Innovation Hub': Zap, 'Cafeteria': Coffee, 'Transport Facility': Bus,
  ...
  const educationCourses = college.courses.filter(c => c.includes('B.A.B.Ed'));

  return (
    <>
      <Helmet><title>{college.name} - Fees, Placements, Ranking 2026 | Top College in Greater Noida</title>
      <meta name="description" content={`${college.name} - Explore fees (${college.fees}), placement packages (${college.highestPackage}), courses offered (B.Tech, MBA, B.Pharm, B.A.B.Ed). Get free counseling for 2026 admissions!`} /></Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2 space-y-6">
              ...
              {activeTab === 'courses' && (
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 rounded-xl p-4"><p className="text-sm text-gray-600"><span className="font-semibold">Note:</span> Hostel fees: ₹1,00,000 - ₹1,15,000/year. Scholarships available for meritorious students.</p></div>
                </div>
              )}
              ...
              {activeTab === 'global' && (
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  ...
                  <div className="mt-6 p-4 bg-blue-50 rounded-lg"><h3 className="font-semibold text-gray-800 mb-2">International Exposure Highlights</h3><ul className="space-y-2 text-sm text-gray-600"><li>✓ IIT Kanpur & IIT Kharagpur Collaborations</li><li>✓ IBM & TCS Certification Programs</li><li>✓ International Study Tours</li><li>✓ Foreign Language Training (French, German, Japanese)</li></ul></div>
                </div>
              )}
            ...
            <div className="lg:col-span-1"><div className="sticky top-16 space-y-6">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border"><h3 className="font-semibold mb-3">Quick Facts</h3><div className="space-y-2 text-sm"><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Established</span><span className="font-medium">{college.established}</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Approvals</span><span className="font-medium">NAAC A, AICTE, NBA</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Student Strength</span><span className="font-medium">{college.students}</span></div><div className="flex justify-between py-2 border-b"><span className="text-gray-500">Faculty Members</span><span className="font-medium">{college.facultyCount}</span></div><div className="flex justify-between py-2"><span className="text-gray-500">Alumni Network</span><span className="font-medium">{college.alumniCount}</span></div></div></div>
            </div></div>
          </div>
        </div>
